  ...
  describe('API', () => {
    describe('Transaction ', () => {
      describe('Should be successful ', () => {
        it('trying to make one click buy', () => {
          const api = `
          ...
          cy.get('.success__title').should('exist')
          cy.get('.success__infoField').should('exist')
          cy.get('.success__infoValue').should('exist')

          cy.get('.success__title').then((elem) => {
            expect(elem[0].innerText).to.eq('Deu tudo certo!')
          })
          cy.get('.success__infoField').then((elem) => {
            expect(elem[0].innerText).to.eq('Valor pago:')
          })
          cy.get('.success__infoValue').then((elem) => {
            expect(elem[0].innerText).contains('100,00')
          })
        })
